...
describe Platform do
  ...
  describe "add_dependency" do
    before(:each) do
      @attr = { :platform_name               => "tta_sub",
                :platform_version            => "1.0.299",
                :ci_job_name                 => "build",
                :branch                      => "master",
                :test_category               => "Unit test",
                :test_sub_category           => "UNIT TEST",
                :test_report_type            => "JUnit",
                :date                        => { :year => "2012", :month => "5", :day => "26", :hour => "07", :minute => "46" },
                :browser_or_device           => "firefox",
                :test_execution_machine_name => "host_pc",
                :os                          => "mac-osx",
                :environment                 => "dev",
                :logDirectory                => double(:original_filename => "abc.zip", :path => "~/Projects/tta/project_logs") }
    end
