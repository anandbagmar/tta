## Sample seed data types

SAMPLE_BROWSER_TYPES= %w( chrome firefox IE opera safari seamonkey k-meleon conqueror maxthon galleon avant netscape iCab camino flock )
SAMPLE_OS_TYPES= ["Mac", "Unix", "Ubuntu", "BeOS", "IRIX", "NeXTSTEP", "MS-DOS", "iOS", "Windows7", "kondara linux", "OSF", "QNX", "SCO", "sunSolaris", "SuSELinux"]
SAMPLE_HOST_NAMES= %w(garima pooja ashwin nikita nikitha tushar matty lava priti sailee sanchari shilpa pranjali akshay aasawaree)
SAMPLE_CI_JOB_NAMES= %w(smoke master regression runtest testomania enternet titanic kanha kaziranga stress opensaysme gir test4treasure quovadis nihao)
SAMPLE_TEST_ENVIRONMENTS = %w(dev qa production uat)
SAMPLE_TEST_CATEGORIES = ["UNIT TEST", "INTEGRATION TEST", "FUNCTIONAL TEST"]
SAMPLE_TEST_REPORT_TYPES = ["JUnit", "Cucumber_HTML", "NUnit"]
SAMPLE_TEST_SUB_CATEGORY = {"UNIT TEST" => "UNIT TEST", "FUNCTIONAL TEST" => ["SMOKE TEST", "SANITY TEST", "REGRESSION TEST"], "INTEGRATION TEST" => "INTEGRATION TEST"}
