class Parser
  ...
  def parse_test_run_record_nunit_xml(meta_id, test_report_type, config_xml)
    return if (REPORTTYPE["test_report_type_mapping"][test_report_type.downcase] != "xml")
    UnitNunitParser.parse(config_xml, meta_id)
  end

  def parse_test_run_record_html(meta_id, test_report_type, config_html)
  ...
  def parse_test_run_record_xml(meta_id, test_report_type, config_xml)
    return if (REPORTTYPE["test_report_type_mapping"][test_report_type.downcase] != "xml")
    ...
    test_suites.each do |test_suite|
      save_test_suite(meta_id, test_report_type, config_xml, test_suite)
    end
  end

  def save_test_suite(meta_id, test_report_type, config_xml, test_suite)
    ...
    save_test_case(test_suite, saved_test_suite_data, test_report_type, config_xml)

  end

  def save_test_case(test_suite, saved_test_suite_data, test_report_type, config_xml)
    if test_report_type == "Rspec JUnit" || test_report_type == "Cucumber JUnit"
      XmlParser.new.saving_junit_test_cases(config_xml, saved_test_suite_data, test_report_type, test_suite)
    elsif test_report_type == "Groovy NUnit"
      GroovyNunitParser.parse(config_xml, saved_test_suite_data, test_suite)
    end
  end
end
