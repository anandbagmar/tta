class Parser
  ...
  def parse_test_run_record_xml(meta_id, test_report_type, extracted_xml)
    ...
    test_suites_xml.each do |test_suite_xml|
      save_test_suite(meta_id, test_report_type, extracted_xml, test_suite_xml)
    end
  end

  def save_test_suite(meta_id, test_report_type, extracted_xml, test_suite_xml)
    ...
    @doc.xpath("//testsuite/testcase").each do |test_case_xml|
      time = XmlParser.new.get_time(test_case_xml, test_suite_xml, test_report_type)
