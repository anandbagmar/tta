class DefectAnalysis
  ...
  def self.get_test_cases(result)
    ...
    result.each do |test_suite_id|
       test_cases << TestCaseRecord.find_by_test_suite_record_id(test_suite_id ,:select=> "class_name , error_msg")
    end
    result_hash = {}
    test_cases.each do |test_case|
      error_message = test_case.error_msg
      result_hash[error_message] = [] unless result_hash.keys.include?(error_message)
      result_hash[error_message] << test_case.class_name
    end
    return result_hash
  end
end
